...
// readClient returns a client for a random Redis instance, as reads can be handled by slaves too
func (ra *RedisAdapter) readClient() (redis.Client, error) {
	if ra.sentinel == nil {
		return ra.client, nil
	}
	master, slaves := ra.sentinel.Addrs()
	slaves = append(slaves, master)
	rand.Seed(time.Now().UnixNano())
	i := rand.Intn(len(slaves))
	client, err := ra.sentinel.Client(slaves[i])
	return client, err
}

// writeClient returns a client for the master Redis instance, as that's the only one that can handle writes
func (ra *RedisAdapter) writeClient() (redis.Client, error) {
	if ra.sentinel == nil {
		return ra.client, nil
	}
	master, _ := ra.sentinel.Addrs()
	client, err := ra.sentinel.Client(master)
	return client, err
}
...
func (ra *RedisAdapter) List(offset, limit int) ([]string, int, error) {
	var (
		res      scanResult
		redisErr resp2.Error
	)
	...
	err = client.Do(redis.Cmd(&res, "SCAN", strconv.Itoa(offset), "COUNT", strconv.Itoa(limit)))
	if errors.As(err, &redisErr) {
